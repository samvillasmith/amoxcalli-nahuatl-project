...
2. `REVIEW_OUTPUT_TEMPLATE.csv` for item-by-item decisions
3. `REVIEW_SUMMARY_TEMPLATE.md` for the short written summary
...
Do not return broad notes without item-level decisions. Broad notes are useful only when they are attached to a specific review item in `REVIEW_OUTPUT_TEMPLATE.csv`.
